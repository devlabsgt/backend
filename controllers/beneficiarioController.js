...
router.post("/beneficiario/crear", async (req, res) => {
  try {
    ...
    res.status(500).json({ mensaje: "Hubo un error al crear el beneficiario" });
  }
});
...
router.get("/beneficiario/:id", async (req, res) => {
  try {
...
router.put("/beneficiario/:id", async (req, res) => {
  const { id } = req.params;
